    ...
    <div ref={ref} style={{ position: "relative", borderTop: "1px solid var(--border)", padding: "8px" }}>
      {open && (
        <div style={{
          ...
          borderRadius: "14px", overflow: "hidden", boxShadow: "0 -8px 32px rgba(0,0,0,0.5)", zIndex: 50,
        }}>
          <div style={{ padding: "14px 16px 12px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <div style={{
              ...
              fontSize: "11px", fontWeight: 600, color: isPro ? "var(--teal)" : "var(--text-3)",
            }}>
          ...
          <div style={{ padding: "6px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
            <button
              ...
              style={{
                width: "100%", display: "flex", alignItems: "center", gap: "10px",
                padding: "9px 12px", borderRadius: "10px", background: "none", border: "none",
                cursor: signingOut ? "not-allowed" : "pointer", color: "#f87171",
                fontSize: "13px", fontWeight: 600, fontFamily: "'Inter','Open Sans',sans-serif",
                transition: "background 0.15s", opacity: signingOut ? 0.5 : 1,
              }}
      ...
      <button
        ...
        style={{
          ...
          borderRadius: "12px", background: open ? "rgba(255,255,255,0.06)" : "none",
      ...
      >
        <div style={{
          ...
          fontSize: "12px", fontWeight: 700, color: "var(--teal)", fontFamily: "'Inter','Open Sans',sans-serif",
        }}>
          {initials}
        </div>
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "var(--bg)" }}>
        ...
        <nav className="relative flex-1 px-3 py-5 space-y-0.5 overflow-y-auto">
          ...
          {[
            ...
            >
              ...
              {item.locked && (
                <span className="type-label" style={{ padding: "2px 6px", borderRadius: "4px", background: "rgba(255,255,255,0.03)", color: "var(--text-4)" }}>SOON</span>
              )}
            </button>
          ))}
        </nav>
      ...
      <main className="flex-1 overflow-y-auto">
        <header className="px-8 py-5 flex items-center gap-4 sticky top-0 z-20" style={{
          background: "rgba(9,9,11,0.88)", backdropFilter: "blur(24px)", borderBottom: "1px solid var(--border)",
        }}>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="relative mb-6">
                  ...
                  return (
                    <div key={lvl.name} className="flex flex-col items-center gap-2">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold"
                        style={{
                          background: isActive ? "var(--teal)" : isPast ? "var(--teal-soft)" : "rgba(255,255,255,0.05)",
                          border: isActive ? "2px solid var(--teal)" : isPast ? "1px solid var(--teal-border)" : "1px solid var(--border)",
                          color: isActive ? "var(--bg)" : isPast ? "var(--teal)" : "var(--text-4)",
                          boxShadow: isActive ? "0 0 16px rgba(31,191,159,0.4)" : "none",
                        }}>
                        {isPast && !isActive ? <CheckCircle2 className="w-4 h-4" /> : levels.findIndex(l => l.name === lvl.name) + 1}
                      </div>
                      <span className="type-meta" style={{ color: isActive ? "var(--teal)" : isPast ? "var(--text-3)" : "var(--text-4)", whiteSpace: "nowrap" }}>
                        {lvl.name}
                      </span>
                    </div>
                  );
              ...
              <div className="h-1 rounded-full overflow-hidden mx-4" style={{ background: "rgba(255,255,255,0.05)" }}>
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: `${Math.min((progress.challenges_completed / 6) * 100, 100)}%` }}
                  transition={{ duration: 1.2, ease: "easeOut", delay: 0.3 }}
                  className="h-full rounded-full"
                  style={{ background: "var(--teal)", boxShadow: "0 0 8px rgba(31,191,159,0.5)" }}
                />
              </div>
            </div>
          </motion.div>
                ...
                {BADGE_DEFINITIONS.map(badge => {
                  ...
                  return (
                    <div key={badge.id} className="flex items-center gap-3 p-3 rounded-xl"
                      style={{
                        background: earned ? `${badge.color}08` : "rgba(255,255,255,0.02)",
                        border: `1px solid ${earned ? badge.color + "20" : "rgba(255,255,255,0.04)"}`,
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="type-meta" style={{ marginTop: "1px", lineHeight: 1.4 }}>
                          {earned && earnedData
                            ? `Earned ${new Date(earnedData.earned_at).toLocaleDateString("en-CA", { month: "short", day: "numeric" })}`
                            : badge.description}
                        </div>
                      </div>
                    </div>
                  ...
                  return (
                    <motion.div key={attempt.id} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                      ...
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                        ...
                        <span style={{ fontSize: "14px", fontFamily: "'Inter','Open Sans',sans-serif", fontWeight: 800, color: typeColor }}>
                          {attempt.total_score}
                        </span>
                      </div>
