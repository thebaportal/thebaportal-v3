...
function TemplateCard({ template }: { template: typeof TEMPLATES[0] }) {
  ...
  return (
    <div style={{ background: "var(--lc-surface)", border: "1px solid var(--lc-border)", borderRadius: "var(--radius-lg)", overflow: "hidden", transition: "border-color .2s" }}
    ...
    >
      <div style={{ padding: "24px 24px 20px" }}>
        ...
        <div style={{ fontFamily: "var(--font-display)", fontSize: 15, fontWeight: 700, color: "var(--lc-text-1)", marginBottom: 8, letterSpacing: "-0.01em", lineHeight: 1.3 }}>
          {template.label}
        </div>
        ...
        {/* Sections toggle */}
        <button onClick={() => setExpanded(e => !e)} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 600, color: "var(--lc-text-3)", background: "none", border: "none", cursor: "pointer", padding: 0, marginBottom: expanded ? 12 : 0, transition: "color .15s" }}
        ...
        {expanded && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {template.sections.map(s => (
              <span key={s} style={{ fontSize: 11, fontWeight: 500, padding: "3px 9px", borderRadius: 6, background: `${template.color}08`, color: "var(--lc-text-3)", border: `1px solid ${template.color}14` }}>{s}</span>
            ))}
          </div>
        )}
      </div>
      ...
      <main style={{ flex: 1, overflowY: "auto" }}>
        <div style={{ padding: "32px 36px" }}>
          ...
          <div style={{ marginBottom: 32 }}>
            <h1 style={{ fontFamily: "var(--font-display)", fontSize: 26, fontWeight: 800, color: "var(--lc-text-1)", letterSpacing: "-0.02em", marginBottom: 6 }}>
              Template Studio
            </h1>
            <p style={{ fontSize: 14, color: "var(--lc-text-3)", lineHeight: 1.6 }}>
              Practical templates for every common BA deliverable. Download, customise to your organisation, and reuse.
            </p>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28, flexWrap: "wrap" }}>
            <div style={{ position: "relative", flex: "0 0 280px" }}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--lc-text-4)" strokeWidth="2.5" strokeLinecap="round" style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)" }}>
          ...
          {/* Stats strip */}
          <div style={{ display: "flex", gap: 20, marginBottom: 28, padding: "14px 20px", background: "var(--lc-surface)", border: "1px solid var(--lc-border)", borderRadius: 12, flexWrap: "wrap" }}>
            {[
          ...
          {/* Grid */}
          {filtered.length === 0 ? (
            <div style={{ textAlign: "center", padding: "60px 0", color: "var(--lc-text-4)", fontSize: 14 }}>
